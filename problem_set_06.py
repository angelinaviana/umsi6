...
def strip_string(list_object, character=None):
    ...
    for i in range(len(list_object)):
        if character:
            a = list_object[i].strip(character)
        else:
            a = list_object[i].strip(' ')
        list_object[i] = a
...
def find_fewest(column, headers, data):
    ...
    for s in data:
        if s[idx] < smallest_value:
            smallest_value = s[idx]
            smallest_unit = []
            smallest_unit.append(s)
        elif s[idx] == smallest_value:
            smallest_unit.append(s)
        else:
            continue
...
#         None
#     """
#     with ?(filepath, ?, encoding=encoding, newl  ) as file_obj:
#         writer = csv.?(file_obj)
#         if :
...
def main():
    ...
    registered_voters = read_csv("./assignments/ps_06/washtenaw_registered_voters.csv")

    print(f'1.2: Registered Voters in Washtenaw County: {registered_voters}')

    # PROBLEM 1.3

    primary_results = read_csv("./assignments/ps_06/washtenaw_2022_primary_results.csv")
